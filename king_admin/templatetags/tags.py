    ...
    for column in admin_class.list_display:
        try:
            ...
            if field_obj.choices:  # choices type
                ...
                if column == 'status':
                    btn = "btn btn-info btn-sm" if column_data == '已报名' else 'btn btn-warning btn-sm'
            else:
                # 通过反射取出
...
# 过滤并生成HTML
@register.simple_tag
def render_filter_ele(filter_field, admin_class, filter_condtions, filter_url, tags):
    activation_tag = ""  # 获取选中的标签

    select_ele = """<dl id="select1">"""
    field_obj = admin_class.model._meta.get_field(filter_field)  # 获取字段对象
    select_ele += """<dt>{}：</dt>""".format(
        admin_class.model._meta.get_field(filter_field).verbose_name)
    # select_ele += """<dd class="select-all selected"><a href="javascript:volid(0);">全部</a></dd>"""
    if field_obj.choices:
        ...
        for choice_item in field_obj.choices:
            ...
            if filter_condtions.get(filter_field) == str(choice_item[0]):
                selected = "selected"
                activation_tag += '''<dd class="{}"><a tag="{}" a="?{}{}={}">{}</a></dd>'''.format(selected,
                                                                                                   filter_field,
                                                                                                   '' if filter_url == "&" else filter_url,
                                                                                                   filter_field,
                                                                                                   choice_item[0],
                                                                                                   choice_item[1])

            # select_ele += '''<option value="{}" {}>{}</option>'''.format(choice_item[0], selected, choice_item[1])
            select_ele += '''<dd class="{}"><a tag="{}" href="?{}{}={}">{}</a></dd>'''.format(selected, filter_field,
                                                                                              '' if filter_url == "&" else filter_url,
                                                                                              filter_field,
                                                                                              choice_item[0],
                                                                                              choice_item[1])
    ...
    if type(field_obj).__name__ in ['DateTimeField', 'DateField']:
        ...
        for item in date_els:
            select_ele += '''<dd class="{}"><a tag="{}" href="?{}{}__gte={}">{}</a></dd>'''.format(selected,
                                                                                                   filter_field,
                                                                                                   '' if filter_url == "&" else filter_url,
                                                                                                   filter_field,
                                                                                                   item[1],
    ...
    if tags == 1:
        ...
        return mark_safe(activation_tag)
...
def build_table_header_column(column, orderby_key, filter_condtions, admin_class):
    ...
    for k, v in filter_condtions.items():
        filters += "&{}={}".format(k, v)
    # <th class="footable-visible footable-sortable footable-sorted">Name<span class="footable-sort-indicator"></span></th>
    ele = ''' <th class="footable-visible footable-sortable footable-sorted"><a href="?{filters}&o={orderby_key}">{column}</a>{sort_icon}</th>'''
    # ele = '''<th><a href="?{filters}&o={orderby_key}">{column}</a>
    # {sort_icon}</th>'''
    if orderby_key:

        if orderby_key.startswith('-'):
            sort_icon = '''<span class="footable-sort-indicator"></span>'''
        else:
            sort_icon = '''<span class="footable-sort-indicator"></span>'''
    ...
    try:
        ...
        return mark_safe(ele)
    return mark_safe(ele.format(filters=filters, orderby_key=orderby_key, column=column_verbose_name, sort_icon=sort_icon))
